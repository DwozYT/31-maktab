...
let appState = {
  settings: {
    ...
    heroDesc: "O'quvchilarimizga zamonaviy bilimlar, raqamli texnologiyalar va yuqori darajadagi ta'lim berish — bizning eng ustuvor maqsadimizdir.",
    statStudents: "1200+",
    statTeachers: "85+",
    statClasses: "40+",
    statAwards: "150+",
    address: "Toshkent shahri, Yunusobod tumani, 17-maktab",
    phone: "+998 (71) 234-56-78",
    email: "info@17-maktab.uz",
  ...
  teachers: [
    {
      id: "t1",
      name: "Tursunova Gulnora Alimovna",
    ...
    },
    {
      ...
      bio: "Ta'lim sifatini oshirish va raqamlashtirish loyihalari rahbari."
    }
  ],
  news: [
    {
      id: "n1",
      title: "Maktabimizda Yangi IT va Robototexnika Laboratoriyasi Ochildi",
      date: "2026-08-10",
      category: "Tadbir",
...
// Sync Data with LocalStorage and Server API
async function loadData() {
  const local = localStorage.getItem("maktab_app_db");
  if (local) {
    try {
...
function saveDataLocally() {
  localStorage.setItem("maktab_app_db", JSON.stringify(appState));
}
...
function renderAllSections() {
  ...
  document.title = `${s.schoolName || "Maktab"} | Rasmiy Portal`;
  document.getElementById("nav-school-name").innerText = s.schoolName || "MAKTAB";
  document.getElementById("footer-school-name").innerText = s.schoolName || "MAKTAB";

  // Hero Section
  document.getElementById("hero-badge-text").innerText = s.heroBadge || "Zamonaviy Ta'lim Maskani";
  document.getElementById("hero-title-main").innerText = s.heroTitle1 || "Bilim Va Kelajak";
  document.getElementById("hero-title-sub").innerText = s.heroTitle2 || "Birlashtirilgan Maskanda";
  ...
  aboutContainer.innerHTML = `
    <div class="glass-card p-8 space-y-4">
      <div class="w-12 h-12 rounded-xl bg-blue-600/20 border border-blue-500/40 text-blue-400 flex items-center justify-center text-2xl">
        🎓
      </div>
  ...
  if (appState.gallery.length === 0) {
    ...
    galleryGrid.innerHTML = appState.gallery.map(g => `
      <div class="glass-card overflow-hidden h-52 relative group cursor-pointer">
        ...
        <div class="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-4 text-center">
...
function handleLoginSubmit(e) {
  ...
  if (found) {
    ...
    sessionStorage.setItem("logged_admin", u);
    closeLoginModal();
    updateAuthNavUI();
    openAdminPanelModal();
  } else {
...
function checkAuthSession() {
  const logged = sessionStorage.getItem("logged_admin");
  if (logged) {
...
function switchAdminTab(tab) {
  ...
  tabs.forEach(t => {
    ...
    if (t === tab) {
      btn.className = "px-4 py-3 border-b-2 border-blue-500 text-blue-400 whitespace-nowrap font-bold";
      content.classList.remove("hidden");
    } else {
      btn.className = "px-4 py-3 border-b-2 border-transparent text-slate-400 hover:text-white whitespace-nowrap font-bold";
...
function renderAdminTeachersTab() {
  ...
  container.innerHTML = appState.teachers.map(t => `
    <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
      ...
      <div class="flex items-center space-x-2 pt-2 border-t border-slate-900">
        <button onclick="deleteTeacher('${t.id}')" class="bg-red-600/30 hover:bg-red-600 text-red-300 hover:text-white px-3 py-1 rounded text-xs font-bold w-full">
          O'CHIRISH
        </button>
      </div>
    </div>
...
function openAddTeacherModal() {
  openCrudModal("Yangi O'qituvchi Qo'shish", `
    <div>
      ...
      <input type="text" id="crud-teacher-name" required class="input-dark" placeholder="Masalan: Alimov Jasur" />
    </div>
    <div>
      <label class="block text-xs text-slate-300 mb-1">Lavozimi (Role)</label>
      <input type="text" id="crud-teacher-role" class="input-dark" placeholder="Masalan: Matematika o'qituvchisi" />
    </div>
    <div>
    ...
    <div>
      <label class="block text-xs text-slate-300 mb-1">Rasm URL (Rasm havolasi)</label>
      <input type="text" id="crud-teacher-photo" class="input-dark" placeholder="https://..." />
    </div>
  `, () => {
    ...
    appState.teachers.push({
      ...
      photo: document.getElementById("crud-teacher-photo").value.trim() || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=600&q=80',
      bio: ""
    });
    saveData();
    renderAllSections();
...
function renderAdminNewsTab() {
  ...
  container.innerHTML = appState.news.map(n => `
    <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
      <div>
        <span class="text-[10px] text-blue-400 font-bold uppercase">${n.date}</span>
        <h5 class="font-bold text-white text-sm line-clamp-2">${n.title}</h5>
      </div>
      <button onclick="deleteNews('${n.id}')" class="bg-red-600/30 hover:bg-red-600 text-red-300 hover:text-white px-3 py-1 rounded text-xs font-bold w-full">
        O'CHIRISH
      </button>
    </div>
  `).join("");
...
function openAddNewsModal() {
  openCrudModal("Yangi Yangilik Qo'shish", `
    ...
    <div>
      <label class="block text-xs text-slate-300 mb-1">Rasm URL</label>
      <input type="text" id="crud-news-image" class="input-dark" placeholder="https://..." />
    </div>
  `, () => {
    ...
    appState.news.unshift({
      ...
      image: document.getElementById("crud-news-image").value.trim() || 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=800&q=80'
    });
    saveData();
    renderAllSections();
    renderAdminNewsTab();
...
function renderAdminGalleryTab() {
  ...
  container.innerHTML = appState.gallery.map(g => `
    <div class="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-2">
      <img src="${g.url}" class="w-full h-24 object-cover rounded-lg" />
      <p class="text-xs text-white truncate font-semibold">${g.title}</p>
      <button onclick="deleteGalleryItem('${g.id}')" class="bg-red-600/30 hover:bg-red-600 text-red-300 hover:text-white px-2 py-1 rounded text-[10px] font-bold w-full">
        O'CHIRISH
      </button>
    </div>
  `).join("");
...
function openAddGalleryModal() {
  openCrudModal("Yangi Foto Qo'shish", `
    ...
    <div>
      <label class="block text-xs text-slate-300 mb-1">Rasm URL</label>
      <input type="text" id="crud-gal-url" required class="input-dark" placeholder="https://..." />
    </div>
  `, () => {
    const title = document.getElementById("crud-gal-title").value.trim();
    const url = document.getElementById("crud-gal-url").value.trim();
    if (!title || !url) return;
...
function renderAdminAchievementsTab() {
  ...
  container.innerHTML = appState.achievements.map(a => `
    <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 flex flex-col justify-between">
      ...
      <button onclick="deleteAchievement('${a.id}')" class="bg-red-600/30 hover:bg-red-600 text-red-300 hover:text-white px-3 py-1 rounded text-xs font-bold w-full">
        O'CHIRISH
      </button>
    </div>
  `).join("");
...
function renderAdminInquiriesTab() {
  ...
  container.innerHTML = appState.inquiries.map((inq, idx) => `
    <div class="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
      ...
      <button onclick="deleteInquiry(${idx})" class="text-[10px] text-red-400 hover:underline">Xabarni o'chirish</button>
    </div>
  `).join("");
}
...
// ADMIN USERS MANAGEMENT (Add new Admin Accounts)
...
function openAddAdminUserModal() {
  openCrudModal("Yangi Admin Qo'shish", `
    <div>
      ...
      <input type="text" id="crud-admin-username" required class="input-dark" placeholder="Masalan: admin2" />
    </div>
    <div>
